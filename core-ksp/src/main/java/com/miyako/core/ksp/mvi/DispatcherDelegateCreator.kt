...
class DispatcherDelegateCreator(private val codeGenerator: CodeGenerator, private val logger: KSPLogger) :
  ...
  override fun visitClassDeclaration(
    ...
    classDeclaration.getDeclaredFunctions().filter { it.isAnnotationPresent(DispatchAction::class) }.forEach {
      ...
      if (parameterSpec != ClassName.bestGuess(funParam.type.resolve().declaration.qualifiedName!!.asString()) ||
        ...
        logger.error(
          "@DispatchAction param or returnType not match ${it.location} function `$funName` param or returnType",
        )
        return
      }
      ...
      buildClass.add(
        "${classDeclaration.qualifiedName?.asString()}_${parameterSpec.simpleName}${returnSpec.simpleName}Dispatcher",
      )
      ...
      actionFunctionList
        .filter {
          if (it.parameters.size == 1 && it.returnType != null) {
            ...
            val msg =
              "delegate fun ${it.simpleName.asString()}" +
            ...
            logger.warn(msg)
            // 检查是否为目标类型或者及其子类
            paramType?.isAssignableFrom(
              actionP.type.resolve(),
            ) == true && returnType?.isAssignableFrom(actionR!!.resolve()) == true
          } else {
            false
          }
        }.forEach {
          ...
          val action =
            it.parameters.firstOrNull()?.type?.resolve()?.declaration?.let {
  ...
  private fun getDefReturnTypeName(
    parameterSpec: ClassName,
    returnSpec: ClassName
  ): TypeName {
    return Return::class.asClassName().parameterizedBy(parameterSpec, returnSpec)
  }

  private fun createOverrideProperty(
    name: String,
    typeName: TypeName
